...
function Matrix.width(m)
    if #m == 0 then return 0
    else return #(m[1]) end
end

function Matrix.height(m)
    return #m
end

function Matrix.isSquare(m)
    return Matrix.width(m) == Matrix.height(m)
end

function Matrix.submatrix(m, i, j)
    local result = {}
    for I = 1, Matrix.height(m) do
        if not (i == I) then
            result[#result+1] = {}
            for J = 1, Matrix.width(m) do
                if not (j == J) then
                    table.insert(result[#result], m[I][J])
                end
            end
        end
    end
...
function Matrix.det(m)
    if not Matrix.isSquare(m) then error() end
    if Matrix.height(m) == 1 then return m[1][1]
    else
        local result = 0
        for i = 1, Matrix.height(m) do
            result = result - alternate(i) * m[i][1] * Matrix.det(Matrix.submatrix(m, i, 1))
        end
        return result
    end
end
...
function Matrix.ortho(left, right, bottom, top, nearVal, farVal)
